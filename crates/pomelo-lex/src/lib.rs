...
//!
//! Heavily inspired by
//! [`rustc_lexer`](https://doc.rust-lang.org/stable/nightly-rustc/rustc_lexer/index.html).
...
///
/// Lexing errors either result in `LexKind::Unknown` or are encoded in extra data
/// in the variant (e.g., `LexKind::String { terminated: bool }`).
/// Error handling is deferred to the parsing stage.
///
/// (Note: we can improve this in the future to be a little more intelligent at
/// the lexing stage, e.g., in lexing numeric literals.)
pub fn lex(src: &str) -> Vec<LexToken> {
    let ls = LexedStr::new(src);
    ls.lex()
}
